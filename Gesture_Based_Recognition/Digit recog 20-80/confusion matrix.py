...
if __name__== "__main__":
    ...
    X_train, y_train = load_dataset("data300/data.txt","data300/output.txt")
    ...
    for epoch in range(num_epochs):
        ...
        val_acc = 0
        val_batches = 0
    newData=load_testdata("test/testdataConv.txt")
    output=load_testoutput("test/datatesto.txt")
    test=len(newData)
    count=0
    for i in range(test):
        data=newData[i]
