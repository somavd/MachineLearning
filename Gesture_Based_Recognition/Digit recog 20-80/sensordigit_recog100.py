...
def load_dataset(filename, filename1):    
    f=open(filename,"r")
    data=f.read()
    data=data.replace('\n',',')
    data=eval(data)
    ...
    X_train, X_test, y_train, y_test = train_test_split(data, op, test_size=0.2, random_state=42)
...
if __name__== "__main__":
    X_train, X_test, y_train, y_test = load_dataset("data100/data.txt","data100/output.txt")
    ...
    while True:
        if(os.path.getsize("data.txt")>0):
            ...
            data=fw.read()
            data=eval(data)
            data=eval(data['data'])[:324]
            data=np.reshape(data,(1,-1,18,18))
            data=data.reshape(-1,1,18,18)
            data=data/np.float32(256)
            x=check_fn(data)
            print("Recognised Gesture..."+str(np.argmax(x)))
