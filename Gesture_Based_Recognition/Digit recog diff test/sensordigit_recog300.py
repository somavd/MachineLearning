...
if __name__== "__main__":
    X_train, y_train = load_dataset("data300/dataall.txt")
    ...
    for epoch in range(num_epochs):
        ...
        val_acc = 0
        val_batches = 0
    newData=load_testdata("data300/testdataConv.txt")
    output=load_testoutput("data300/datatesto.txt")
    test=len(newData)
    count=0
    for i in range(test):
        data=newData[i]
        ans=check_fn([data])
        x=str(np.argmax(max(ans)))
        y=str(output[i])
